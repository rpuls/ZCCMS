...
$loginUrl = "https://www.facebook.com/dialog/HEMMELIG&redirect_uri=". urlencode(".com/lookup/lookup.php?scope=$scope");
...
if(($FB_page_id == null) || ($FB_page_id == 0) || (!$FB_page_id)){
	error_log("empty FB_page_id in manage_form_processor.php ");
	header('HTTP/1.1 307');
    echo '/lookup/lookup.php';
	exit;
}

$is_this_the_global_admin = false;
$is_this_the_page_admin = false;
$FB_page_name = '';

if(1==1){
// ----------- admin, no need to check the id, only retrieve the page_name and the picture
    ...
    try {
        ...
        //$response = $fb->get('', $FacebookSession);
        $page_object = $response->getGraphPage();

        //$FB_page_name = $page_object->getProperty('name');
	...
	try {
		$GO_manager = (new FacebookRequest(
			$FacebookSession, 'GET', "/$FB_userid/accounts?fields=id"
		))->execute()->getGraphObject();
		$FB_manager_array1 = $GO_manager->asArray();
		$FB_manager_array2 = $FB_manager_array1['data'];
	...
	foreach($FB_manager_array2 as $result){
		if($result->id == $FB_page_id){
			$is_this_the_page_admin = true;
		}
	}
...
// ----------- and get some more site details from the database.
...
if(($ArPlaceDetails['FB_ID'] != $FB_page_id) OR ($ArPlaceDetails['ID'] != $zite_id)){
    error_log("FBUser $FB_userid tried to edit site ". safe_get_GET_OR_POST('fb_id').", $zite_id , but the id's in the database do not seem to match up.");
    header('HTTP/1.1 403');
	echo("You are not alowed to edit this page.");
	exit();
}
